class thiss::pyff_prod($pyff_version="thiss",$output="/etc/thiss/metadata.json",$output_trust="/etc/thiss/metadata_sp.json") {
  $image_tag = "docker.sunet.se/pyff:${pyff_version}"
  $pipeline = hiera("pyff_pipeline_prod")
  ...
  file {"/usr/local/sbin/run-pyff":
     content => template("thiss/pyff/run-pyff_prod.erb"),
     mode    => '0755'
  } ->
